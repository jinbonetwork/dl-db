...
const _fieldAttrs = {
	id: {type: 'meta'}, uid: {type: 'meta'}, created: {type: 'meta'},
...
const _fname = {
	id: 'id', uid: 'uid', created: 'created', subject: 'title', content: 'content',
...
const _sFname = {
	id: 'id', uid: 'uid', created: 'created', title: 'subject', content: 'content',
...
const _convertToDoc = (sDoc) => {
	...
	for(let fn in _emptyDocument){
		...
		if(sDoc[sFn]){
			switch(fAttr.type){
				case 'meta': case 'char': case 'date':
					document[fn] = sDoc[sFn]; break;
				case 'taxonomy':
					document[fn] = [];
					for(let tid in sDoc[sFn]) document[fn].push(parseInt(tid));
					if(fAttr.multiple === false) document[fn] = document[fn][0];
					break;
				case 'image': case 'file':
					document[fn] = [];
					for(let fid in sDoc[sFn]){
						sDoc[sFn][fid].fid = fid;
						document[fn].push(sDoc[sFn][fid]);
					}
					if(fAttr.multiple === false) document[fn] = document[fn][0];
					break;
			}
