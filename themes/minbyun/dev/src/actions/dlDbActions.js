import { SHOW_MESSAGE, HIDE_MESSAGE, RECEIVE_USER_FIELD_DATA, RECEIVE_DOC_FIELD_DATA, RECEIVE_ROOT_DATA,
	...
	CHANGE_USER_REGIST, FOCUSOUT_USER_REGIST, FOCUSIN_USER_REGIST, COMPLETE_USER_REGIST, DECOMPLETE_USER_REGIST, USER_DUPLICATE,
	SUBMIT_USER_REGIST, INITIALIZE_DOCFORM, SELECT_IMAGE, RECEIVE_COURTS, TOGGLE_REPORT_FORM, CHANGE_REPORT
} from '../constants';
...
const actionCreators = {
	...
	submitDocForm(args){ return (dispatch) => {
		const {doc, oldDoc, files, oldFiles, docFormData, fileFormData, afterUpload, isAdmin, afterSave} = args;
	...
	submitUserRegist(args){ return (dispatch) => {
		...
		api.submitUserRegist( pfFormData,
			...
			(error) => {
				dispatch({type: DECOMPLETE_USER_REGIST});
				dispatchError(dispatch, error);
			}
		);
	}},
	...
	toggleReportForm(){
		return {type: TOGGLE_REPORT_FORM};
	},
	changeReport(report){
		return {type: CHANGE_REPORT, report};
	},
	sendReport(args){ return (dispatch) => {
