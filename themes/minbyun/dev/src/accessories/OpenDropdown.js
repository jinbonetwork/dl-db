...
class OpenDropdown extends Component {
	constructor(){
		super();
		this.state = {
...
//		this.setInitSize();
	...
	setInitSize() {
		...
		const width = this.props.initWidth ? this.props.initWidth : 100;
	...
	handleKeyDown(which, arg1st, arg2nd, arg3nd){
		if(which == 'head'){
			...
			if(event.key == 'Enter') {
				...
				this.setSize();
				this.setState({isUnfolded: true, focused: 1});
	...
	render(){
		let className = (this.props.className ? 'dropdown opendropdown '+this.props.className : 'dropdown opendropdown');
